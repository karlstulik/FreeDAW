...
function showAddTrackDialog() {
  ...
  Object.keys(pluginTypes).forEach(type => {
    const option = document.createElement('option');
    option.value = type;
    option.textContent = pluginTypes[type].name;
  ...
  createBtn.onclick = () => {
    ...
    createTrack('Track ' + (tracks.length + 1), pluginType);
...
async function togglePlay() {
  ...
  if (!isPlaying.value) {
    ...
    schedulerInterval = setInterval(schedule, 50);
  } else {
