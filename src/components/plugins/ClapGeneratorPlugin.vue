...
import { ClapGeneratorPlugin } from '@/plugins/ClapGeneratorPlugin.js'
...
const presetOptions = computed(() => {
  return Object.keys(ClapGeneratorPlugin.presets).map(name => ({
    title: name,
    value: name
  }))
})
...
const loadPreset = async (presetName) => {
  if (presetName && ClapGeneratorPlugin.presets[presetName]) {
    isLoadingPreset.value = true
    const preset = ClapGeneratorPlugin.presets[presetName]
...
const auditionPreset = (presetName) => {
  if (presetName && ClapGeneratorPlugin.presets[presetName]) {
    ...
    const tempPlugin = new ClapGeneratorPlugin(props.plugin.track, presetName)
    // Play a short sample
